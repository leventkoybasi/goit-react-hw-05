...
function Movie() {
  ...
  useEffect(() => {
    const fetchMovieData = async () => {
      try {
        const data = await fetchMovies(BASE_URL, `${ENDPOINTS.MOVIE_DETAILS(id)}`);
        setMovie(data);

        const movieCredit = await fetchMovies(BASE_URL, `${ENDPOINTS.MOVIE_CREDITS(id)}`);
        const cast = movieCredit.cast;
        setCast(cast);

        const movieReview = await fetchMovies(BASE_URL, `${ENDPOINTS.MOVIE_REVIEWS(id)}`);
      ...
      <div className='row my-5'>
        <div className='col-sm-12 col-md-12 col-lg-4'>
          ...
          <ul className='fs-4 px-0 py-3' style={{ listStyle: 'none' }}>
            <li>
              <NavLink className='text-primary text-decoration-none py-1' to={`/movie/${id}/cast`}>
                View Cast
              </NavLink>
            </li>
            <li>
